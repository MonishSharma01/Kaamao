  ...
  return (
    ...
    >
      <div className="flex flex-col h-full">
        {/* Logo Area */}
        <div className={`flex items-center h-20 px-6 border-b border-white/10 ${collapsed ? 'justify-center' : 'justify-between'}`}>
          {!collapsed && (
            <div className="flex items-center gap-2">
              ...
              <span className="font-bold text-xl tracking-tight">ServiceHub</span>
            </div>
          )}
          {collapsed && (
            <div className="relative w-8 h-8">
          ...
          {menuItems.map((item) => {
            ...
            return (
              <button
                ...
                `}
              >
                <Icon className={`h-5 w-5 transition-transform duration-200 group-hover:scale-110 ${item.active ? 'text-white' : ''}`} />
                {!collapsed && (
...
export default function Home() {
  ...
  const menuItems: MenuItem[] = [
    { id: 'dashboard', label: 'Dashboard', icon: LayoutDashboard, active: currentPage === 'dashboard' },
    { id: 'create-service', label: 'Create Service', icon: Briefcase, active: currentPage === 'create-service' },
    { id: 'find-services', label: 'Find Services', icon: Search, active: currentPage === 'find-services' },
    { id: 'nearby', label: 'Nearby Providers', icon: MapPin, active: currentPage === 'nearby' },
    { id: 'analytics', label: 'Analytics', icon: BarChart3, active: currentPage === 'analytics' },
    { id: 'profile', label: 'Profile', icon: User, active: currentPage === 'profile' },
    { id: 'settings', label: 'Settings', icon: SettingsIcon, active: currentPage === 'settings' },
  ];

  const renderPage = () => {
    switch (currentPage) {
      case 'dashboard': return <DashboardOverview />;
      case 'create-service': return <CreateService />;
      case 'find-services': return <FindServices />;
      case 'nearby': return <NearbyProviders />;
        ...
        <header className="sticky top-0 z-10 bg-white/70 backdrop-blur-md border-b border-white/50 shadow-sm">
          <div className="px-8 py-4 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <button type="button" aria-label="Notifications" title="Notifications" className="p-2 rounded-xl hover:bg-white/60 transition relative">
